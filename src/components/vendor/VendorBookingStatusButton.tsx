...
type VendorBookingStatusButtonProps = {
  bookingId: string;
...
export default function VendorBookingStatusButton({
  bookingId,
  currentStatus,
  vendorNote,
}: VendorBookingStatusButtonProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/vendor/bookings/status", {
        method: "PATCH",
        headers: {
      ...
      console.error("UPDATE_VENDOR_BOOKING_STATUS_ERROR", error);
