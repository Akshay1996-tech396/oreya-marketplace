  ...
  try {
    ...
    if (user.role !== "ADMIN") {
      return NextResponse.json(
        {
          success: false,
          message: "Only administrators can access restaurant operating hours.",
          hours: [],
        },
        { status: 403 }
      );
    ...
    console.error("ADMIN_RESTAURANT_OPERATING_HOURS_GET_ERROR", error);
  ...
  try {
    ...
    if (user.role !== "ADMIN") {
      return NextResponse.json(
        {
          success: false,
          message: "Only administrators can update restaurant operating hours.",
          hours: [],
        },
        { status: 403 }
      );
    ...
    console.error("ADMIN_RESTAURANT_OPERATING_HOURS_UPDATE_ERROR", error);
