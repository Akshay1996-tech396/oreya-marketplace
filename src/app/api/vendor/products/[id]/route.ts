...
function parseOptionValues(value: unknown) {
...
function parseProductOptions(options: unknown): ParsedProductOption[] {
  ...
  const parsedOptions: ParsedProductOption[] = [];
  ...
  options.forEach((item, index) => {
    ...
    const values = parseOptionValues(row.values);
    ...
    parsedOptions.push({
      ...
      sortOrder: Number.isFinite(Number(row.sortOrder))
        ? Math.max(0, Number(row.sortOrder))
        : index,
    });
  });
...
function parseVariantOptions(value: unknown) {
  ...
  Object.entries(row).forEach(([key, optionValue]) => {
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const currency =
      String(body.currency || "AED").trim().toUpperCase() || "AED";
    ...
    if (variationValidationMessage) {
      return NextResponse.json(
        { message: variationValidationMessage },
        { status: 400 }
      );
    }
    ...
    console.error("VENDOR_PRODUCT_UPDATE_ERROR", error);
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    console.error("VENDOR_PRODUCT_STATUS_ERROR", error);

    return NextResponse.json(
      {
        success: false,
        message: "Unable to change the product status at this time.",
      },
      { status: 500 }
    );
  }
