...
type VendorAuthorizationResult = {
...
async function getApprovedVendor(): Promise<VendorAuthorizationResult> {
  ...
  if (user.role !== "VENDOR") {
    return {
      response: errorResponse(
        "Only vendors can access this resource.",
        403
      ),
      vendor: null,
    };
  ...
  try {
    ...
    if (
      ...
      return (
        authorization.response ??
        errorResponse(
          "Unable to authorize this request.",
          403
        )
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    console.error(
      "VENDOR_RESTAURANT_DELETE_ERROR",
      error
    );
    ...
    if (
      ...
      if (error.code === "P2003") {
        return errorResponse(
          "This restaurant cannot be deleted because it has related records.",
          409
        );
      }
    ...
    return errorResponse(
      "Unable to delete the restaurant at this time.",
      500
    );
  }
}