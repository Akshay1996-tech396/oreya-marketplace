...
export async function GET(_request: Request, { params }: RouteContext) {
  try {
    ...
    console.error("VENDOR_RESTAURANT_TABLES_GET_ERROR", error);
  ...
  try {
    ...
    if (user.role !== "VENDOR") {
      return NextResponse.json(
        {
          success: false,
          message: "Only vendors can create restaurant tables.",
        },
        { status: 403 }
      );
    }
    ...
    console.error("VENDOR_RESTAURANT_TABLE_CREATE_ERROR", error);
